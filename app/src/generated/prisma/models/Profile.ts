...
export interface ProfileDelegate<ExtArgs extends runtime.Types.Extensions.InternalArgs = runtime.Types.Extensions.DefaultArgs, GlobalOmitOptions = {}> {
   ...
   * })
   * 
   * // Create many Profiles and only return the `name`
   * const profileWithNameOnly = await prisma.profile.createManyAndReturn({
   *   select: { name: true },
   *   data: [
   *     // ... provide data here
   *   ]
   ...
   * })
   * 
   * // Update zero or more Profiles and only return the `name`
   * const profileWithNameOnly = await prisma.profile.updateManyAndReturn({
   *   select: { name: true },
   *   where: {
   *     // ... provide filter here
   *   },
